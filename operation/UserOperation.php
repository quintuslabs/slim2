...
class UserOperation extends DbOperation
{
    ...
    public function register($name, $email, $password, $mobile)
    {
        ...
        if (!$this->isUserExists($email, $mobile)) {
            ...
            $stmt = $this->con->prepare("INSERT INTO users(name, email, password, mobile, api_key,date) values(?,?,?,?,?,?)");
            $stmt->bind_param("ssssss", $name, $email, $password, $mobile, $apikey, $dateTime);
            $result = $stmt->execute();
            $stmt->close();
            if ($result) {
                return 0;
    ...
    public function updateUsers($id, $gender,$dob)
    {
        $stmt = $this->con->prepare("UPDATE users SET gender = ?, dob = ? WHERE id = ?");
        $stmt->bind_param("sss", $gender, $dob, $id);
        if ($stmt->execute()) {
            return 0;
        } else {
            return 1;
